...
def _apply_online_warehouse_stock(catalog):
    ...
    for product in catalog:
        ...
        for variant in product.get("variants", []):
            available = balances.get(variant["id"], 0); variant["stock"] = available; variant["available"] = available > 0; total_available += available
...
def catalog_context():
    ...
    for product in catalog: product["url"] = reverse("storefront:product_detail", kwargs={"slug": product["slug"]})
    ...
    return {"catalog": catalog, "products": catalog, "hero": mock_data.HERO_SLIDES[0], "cart_summary": {"count": 0, "subtotal": 0, "total": 0}, "categories": category_filters(), "brands": brand_filters(), "tracking": mock_data.TRACKING_ORDER, "featured_products": featured, "related_products": catalog[:4], "recommended_products": catalog[3:9] if len(catalog) > 3 else catalog[:6], "routes": routes, "store_data": {"products": browser_products, "listing_products": browser_products, "cart": [], "wishlist": mock_data.DEFAULT_WISHLIST, "coupons": {}, "slides": [{**slide, "img": static(slide["image"]), "href": routes["products"]} for slide in mock_data.HERO_SLIDES]}}
