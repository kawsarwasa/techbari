...
def _product_form_context(page_name, request, form, product=None):
    ...
    context.update(
        ...
        specifications=_posted_specs(
            request,
            [{"name": spec.name, "value": spec.value} for spec in product.specifications.all()]
            if product
            else DEFAULT_SPECS,
        ),
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            save_product_bundle(
                form,
                image_files,
                request.POST.getlist("spec_name"),
                request.POST.getlist("spec_value"),
            )
            return redirect(reverse("backoffice:products") + "?notice=created")
    return render(
        request,
        "backoffice/pages/products/product_add.html",
        _product_form_context("product_add", request, form),
    )
...
def product_edit(request):
    ...
    return render(
        request,
        "backoffice/pages/products/product_edit.html",
        _product_form_context("product_edit", request, form, product),
    )
...
def categories(request):
    if request.method == "POST" and request.POST.get("action") == "delete":
        ...
        try:
            ...
            return redirect(reverse("backoffice:categories") + "?error=category-used")
    ...
    context["statistics"] = _replace_stat_values(
        context["statistics"],
        [
            {"label": "Total Categories", "value": str(len(rows)), "trend": "Live"},
            {"label": "Active Categories", "value": str(active), "trend": "Live"},
            {"label": "Top-level Categories", "value": str(parent_count), "trend": "Live"},
            {"label": "Inactive Categories", "value": str(len(rows) - active), "trend": "Live"},
        ],
    )
...
def category_add(request):
    ...
    if request.method == "POST" and form.is_valid():
        form.save()
        return redirect(reverse("backoffice:categories") + "?notice=category-saved")
    context = _base_context("category_add", request)
    context.update(form=form, is_edit=bool(instance), category_obj=instance)
...
def brands(request):
    if request.method == "POST" and request.POST.get("action") == "delete":
        ...
        try:
            ...
            return redirect(reverse("backoffice:brands") + "?error=brand-used")
    ...
    context["statistics"] = _replace_stat_values(
        context["statistics"],
        [
            {"label": "Total Brands", "value": str(len(rows)), "trend": "Live"},
            {"label": "Active Brands", "value": str(active), "trend": "Live"},
            {"label": "Featured Brands", "value": str(featured), "trend": "Live"},
            {"label": "Inactive Brands", "value": str(len(rows) - active), "trend": "Live"},
        ],
    )
...
def brand_add(request):
    ...
    if request.method == "POST" and form.is_valid():
        form.save()
        return redirect(reverse("backoffice:brands") + "?notice=brand-saved")
    context = _base_context("brand_add", request)
    context.update(form=form, is_edit=bool(instance), brand_obj=instance)
...
def _ensure_product_primary_image(product, preferred=None):
    if product.images.filter(role=ProductImage.Role.PRIMARY).exists():
        return
    candidate = (
        product.images.exclude(pk=preferred.pk if preferred else None).order_by("sort_order", "id").first()
        or preferred
...
def media(request):
    notice_map = {
        "saved": "Product image saved successfully.",
        "deleted": "Product image deleted successfully.",
        "primary": "Primary image updated.",
        "detail": "Detail image updated.",
    }
    if request.method == "POST":
        ...
        if action == "set_primary":
            ...
            image.role = ProductImage.Role.PRIMARY
        ...
        if action == "set_detail":
            ...
            image.role = ProductImage.Role.DETAIL
            image.save(update_fields=["role"])
...
def media_form(request):
    ...
    context.update(
        ...
        image_obj=instance,
        image_preview=_image_preview_url(instance) if instance else "",
...
def specifications(request):
    ...
    if request.method == "POST" and request.POST.get("action") == "delete":
        specification = get_object_or_404(
            ...
            pk=request.POST.get("specification_id"),
        )
