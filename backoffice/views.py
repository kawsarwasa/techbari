...
def page(request, page_name="dashboard"):
    ...
    if entity:
        try: record_id = int(request.GET.get("id", 1))
        except ValueError: raise Http404("Record not found")
        ...
        if record is None:
            if request.GET.get("local") != "1": raise Http404("Record not found")
            record = dict(records[0])
        if request.GET.get("local") == "1": context["admin_data"]["detail"] = {"entity": entity, "id": record_id}
        ...
        if entity == "orders":
            ...
            if record.get("date"): record["date_label"] = date.fromisoformat(record["date"]).strftime("%b %d, %Y")
            record.setdefault("payment", "Cash on Delivery")
            product = next((p for p in context["admin_data"]["pos_products"] if p["name"] == record["items"]), {})
            record.update(category=product.get("category", "Electronics"), sku=product.get("sku", ""), image=product.get("image", context["admin_data"]["default_image"]).removeprefix("/static/"))
            if record_id == 1:
                record.update(email="tanvir.hasan@gmail.com", phone="+880 1712 345678", image="admin/images/airpods-pro2.webp", sku="IPH15-128-BLK")
        else:
            record["customer_no"] = f"CUST{record_id:03d}"
            record["initials"] = "".join(word[0] for word in record["name"].split())
            context["customer_history"] = CUSTOMER_HISTORY.get(record_id, [{**order, "date_label": date.fromisoformat(order["date"]).strftime("%b %d, %Y"), "linked": True} for order in context["orders"] if order["customer"] == record["name"]])
