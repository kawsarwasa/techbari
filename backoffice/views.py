...
def page(request, page_name="dashboard"):
    ...
    # Browser-only form records are resolved by the existing localStorage scripts.
    ...
    if entity:
        ...
        if record is None:
            if request.GET.get("local") != "1":
                raise Http404("Record not found")
            # The server cannot read browser storage. Render the approved shell;
            # the existing browser demo fills its own record into the marked fields.
        ...
        if entity == "orders":
            ...
            record.update(category=product.get("category", "Electronics"), sku=product.get("sku", ""),
                          image=product.get("image", context["admin_data"]["default_image"]).removeprefix("/static/"))
            if record_id == 1:
                record.update(email="tanvir.hasan@gmail.com", phone="+880 1712 345678",
                              image="admin/images/airpods-pro2.webp", sku="IPH15-128-BLK")
        else:
            record["customer_no"] = f"CUST{record_id:03d}"
            record["initials"] = "".join(word[0] for word in record["name"].split())
            context["customer_history"] = CUSTOMER_HISTORY.get(record_id, [
                {**order, "date_label": date.fromisoformat(order["date"]).strftime("%b %d, %Y"), "linked": True}
                for order in context["orders"] if order["customer"] == record["name"]
            ])
...
def legacy_page(request, page):
    name = "dashboard" if page == "index" else next(
        (name for name, info in PAGES.items() if info["legacy"] == page), None)
    if name is None:
        raise Http404("Dashboard page not found")
