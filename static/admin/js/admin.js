...
(function(){
  ...
  document.addEventListener('click',async e=>{const b=e.target.closest('[data-delete-row]');if(!b)return;const ent=b.dataset.deleteRow,id=Number(b.dataset.id);const ok=await window.TechBariDeleteConfirm.ask({kind:'Demo Record',name:ent,question:'Delete this record from the static demo?',confirmLabel:'Delete Record',warning:'This removes only the current static demo record.'});if(ok){set(ent,get(ent).filter(x=>x.id!==id));const table=$(`table[data-crud-table="${ent}"]`);if(table)renderTable(table);toast('Record deleted')}});
