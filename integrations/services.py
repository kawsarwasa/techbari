...
def _notification(*, kind, severity, title, message, link="", reference_type="", reference_id="", dedupe_key=None):
    defaults = {"kind": kind, "severity": severity, "title": str(title)[:180], "message": str(message)[:500], "link": str(link or "")[:500], "reference_type": str(reference_type or "")[:50], "reference_id": str(reference_id or "")[:100]}
    if dedupe_key:
...
def _outbound(*, channel, event_type, idempotency_key, recipient="", subject="", body="", payload=None, reference_type="", reference_id=""):
    defaults = {"channel": channel, "event_type": event_type, "recipient": str(recipient or "")[:500], "subject": str(subject or "")[:255], "body": str(body or ""), "payload": payload or {}, "reference_type": str(reference_type or "")[:50], "reference_id": str(reference_id or "")[:100]}
    try:
...
def _purchase_payload(order):
    return [{"item_id": item.sku_snapshot, "item_name": item.product_snapshot, "item_variant": item.variant_snapshot, "price": float(item.unit_price), "quantity": int(item.quantity)} for item in order.items.all()]
...
def _enqueue_purchase_analytics(order, config):
    ...
    if config.meta_capi_enabled and config.meta_pixel_enabled and config.meta_pixel_id:
        ...
        payload = {"pixel_id": config.meta_pixel_id, "event": {"event_name": "Purchase", "event_time": int(timezone.now().timestamp()), "event_id": event_id, "action_source": "website", "user_data": user_data, "custom_data": {"currency": "BDT", "value": float(order.grand_total or Decimal("0.00")), "order_id": order.order_number, "contents": [{"id": row["item_id"], "quantity": row["quantity"], "item_price": row["price"]} for row in items], "content_type": "product"}}}
    ...
    if config.ga4_server_enabled and config.ga4_enabled and config.ga4_measurement_id:
        payload = {"measurement_id": config.ga4_measurement_id, "client_id": f"{order.pk}.{int(order.created_at.timestamp()) if order.created_at else int(timezone.now().timestamp())}", "events": [{"name": "purchase", "params": {"transaction_id": order.order_number, "currency": "BDT", "value": float(order.grand_total or Decimal("0.00")), "shipping": float(order.shipping_charge or Decimal("0.00")), "items": items}}]}
...
def enqueue_order_event(order, event_type):
    ...
    if config.notify_order_events:
        _notification(kind=Notification.Kind.ORDER, severity=severity, title=f"Order {order.order_number}: {label}", message=f"{order.customer_name} · ৳ {order.grand_total:.2f}", link=reverse("backoffice:order_detail_id", args=[order.pk]), reference_type="sales_order", reference_id=order.order_number, dedupe_key=f"order:{event_type}:{order.order_number}")
        if order.status != order.Status.DRAFT:
            _order_customer_channels(order, config, f"order_{event_type}", f"TechBari order {order.order_number}", f"TechBari order {order.order_number} is {label}. Total: ৳ {order.grand_total:.2f}.")
...
def enqueue_payment_event(payment, event_type="completed"):
    ...
    _notification(kind=Notification.Kind.PAYMENT, severity=severity, title=f"Payment {payment.get_status_display()}", message=f"{reference} · ৳ {payment.amount:.2f}" + (f" · {order.order_number}" if order else ""), link=reverse("backoffice:payment_detail", args=[payment.pk]), reference_type="payment", reference_id=reference, dedupe_key=f"payment:{event_type}:{reference}")
    if order and payment.status == payment.Status.COMPLETED:
        _order_customer_channels(order, config, f"payment_{reference}", f"Payment received - {order.order_number}", f"Payment received for TechBari order {order.order_number}: ৳ {payment.amount:.2f}.")
...
def _courier_env(code, suffix):
    return str(os.getenv(f"COURIER_{code}_{suffix}", "") or "").strip()
...
def enqueue_shipping_event(shipment, event_type="updated"):
    ...
    api_base_url = _courier_env(shipment.courier.code, "API_BASE_URL")
    if event_type == "created" and config.courier_api_enabled and shipment.courier.api_enabled and api_base_url:
        payload = {"shipment_id": shipment.pk, "shipment_no": shipment.shipment_no, "courier_code": shipment.courier.code, "api_base_url": api_base_url, "api_create_path": _courier_env(shipment.courier.code, "CREATE_PATH") or "/shipments", "token_env": f"COURIER_{shipment.courier.code}_API_TOKEN", "order": {"order_number": shipment.order.order_number, "recipient_name": shipment.order.shipping_name, "recipient_phone": shipment.order.shipping_phone, "recipient_email": shipment.order.shipping_email, "address": shipment.order.shipping_summary, "cod_amount": float(shipment.cod_expected), "parcel_count": shipment.parcel_count, "weight_kg": float(shipment.weight_kg)}}
