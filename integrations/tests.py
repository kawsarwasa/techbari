...
class IntegrationBase(TestCase):
    def setUp(self):
        ...
        self.category = Category.objects.create(name="Integration Phones", slug="integration-phones")
        self.brand = Brand.objects.create(name="Integration Brand", slug="integration-brand")
        self.product = Product.objects.create(
            public_id="integration-phone",
        ...
        self.order = save_sales_order(
            header_data={
                ...
                "shipping_district": self.customer.district,
...
class IntegrationServiceTests(IntegrationBase):
    def test_singleton_and_public_tracking_config_never_expose_secrets(self):
        ...
        config.meta_pixel_id = "123456"
        config.ga4_enabled = True
        config.ga4_measurement_id = "G-TEST123"
        config.save()
    ...
    def test_notification_read_is_per_user(self):
        ...
        first = user_model.objects.create_user(username="first", password="Testpass123!")
        second = user_model.objects.create_user(username="second", password="Testpass123!")
        notification = Notification.objects.create(kind=Notification.Kind.ORDER, title="Order", message="Created")
        self.assertEqual(unread_notifications_for(first).count(), 1)
    ...
    def test_order_created_enqueues_meta_and_ga_purchase_with_same_stable_identity(self):
        config = get_integration_settings()
        config.email_enabled = False
        config.meta_pixel_enabled = True
        config.meta_pixel_id = "999"
        config.meta_capi_enabled = True
        config.ga4_enabled = True
        config.ga4_measurement_id = "G-TEST"
        config.ga4_server_enabled = True
        config.save()
        ...
        self.assertEqual(meta.reference_id, self.order.order_number)
    ...
    def test_failed_delivery_is_retryable_and_creates_alert(self):
        message = OutboundMessage.objects.create(
            channel=OutboundMessage.Channel.SMS,
            event_type="test",
            recipient="+8801711111111",
            body="Test",
            payload={"url": "https://example.invalid/sms"},
            idempotency_key="sms:test:1",
        )
    ...
    def test_courier_booking_uses_environment_configuration_not_database_secret(self):
        config = get_integration_settings()
        config.courier_api_enabled = True
        config.notify_shipping_events = False
        config.save()
        ...
        with patch.dict(os.environ, {"COURIER_FAST_API_BASE_URL": "https://courier.example/api", "COURIER_FAST_CREATE_PATH": "/orders"}, clear=False):
        ...
        self.assertNotIn("token", json.dumps(outbound.payload).lower().replace("token_env", ""))
...
class CourierWebhookTests(IntegrationBase):
    def test_invalid_signature_is_rejected(self):
        ...
        with patch.dict(os.environ, {"COURIER_HOOK_WEBHOOK_SECRET": "topsecret"}, clear=False):
            response = self.client.post(
                reverse("integrations:courier_webhook", args=[courier.code]),
                data=json.dumps({"tracking_id": "ABC"}),
                content_type="application/json",
                HTTP_X_TECHBARI_SIGNATURE="wrong",
            )
    ...
    def test_valid_signature_can_update_tracking_location(self):
        ...
        payload = json.dumps({"tracking_id": "TRK-1", "reference": "REF-1", "location": "Dhaka Hub", "note": "Scanned"}).encode("utf-8")
        ...
        with patch.dict(os.environ, {"COURIER_SIGNED_WEBHOOK_SECRET": "topsecret"}, clear=False):
            response = self.client.post(
                ...
                data=payload,
                content_type="application/json",
                HTTP_X_TECHBARI_SIGNATURE=f"sha256={signature}",
            )
...
class IntegrationPermissionTests(TestCase):
    def setUp(self):
        user_model = get_user_model()
        self.admin = user_model.objects.create_superuser(username="intadmin", email="admin@example.com", password="Testpass123!")
        self.cashier = user_model.objects.create_user(username="intcashier", password="Testpass123!", is_staff=True)
