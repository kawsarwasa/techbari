...
def available_sku(base, *, exclude_pk=None):
    candidate = base
    suffix = 2
    qs = ProductVariant.objects.all()
    if exclude_pk:
        qs = qs.exclude(pk=exclude_pk)
    while qs.filter(sku__iexact=candidate).exists():
        tail = f"-{suffix}"
...
def has_business_history(variant):
    return (
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--reset", action="store_true", help="Delete all catalog data first, then seed the demo catalog. Refuses to run when protected business history exists.")
        parser.add_argument("--refresh-demo", action="store_true", help="Refresh demo presentation fields without recreating existing SKU identities or deleting business-linked variants.")
    ...
    def handle(self, *args, **options):
        ...
        if reset:
            try:
                ...
                raise CommandError(
                    "Catalog reset refused because products/variants are referenced by business history. "
                    "Use --refresh-demo to preserve stable IDs and transaction history."
                ) from exc
        ...
        for index, name in enumerate(category_names):
            ...
            defaults = {
                ...
                "static_image_path": meta.get("image", ""),
                "is_active": True,
                "sort_order": index,
            }
        ...
        for index, row in enumerate(product_rows):
            ...
            defaults = {
                ...
                "is_featured": index < 6 or row.get("badge") == "Bestseller",
            ...
            for variant_index, variant_row in enumerate(ordered_variants):
                ...
                symbol = variant_row.get("symbol") or SYMBOLS.get(name, "")
                desired_sku = sku_for(row["id"], variant_index)
                variant = product.variants.filter(name__iexact=name).order_by("id").first()
                if variant:
                    business_linked = has_business_history(variant)
                    if business_linked:
                        preserved_variant_count += 1
                    variant.symbol = symbol
                    if not business_linked:
                        variant.stock_quantity = int(row.get("stock", 0)) if variant_index == 0 else 0
                ...
                touched_variants.append(variant)

            if not product.variants.filter(is_default=True, is_active=True).exists():
                replacement = next((variant for variant in touched_variants if variant.is_active), None)
                replacement = replacement or product.variants.filter(is_active=True).order_by("id").first()
                if replacement:
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Catalog seeded: {created_count} products created, {existing_count} existing preserved, "
                f"{len(categories)} categories, {len(brands)} brands."
            )
        )
        if preserved_variant_count:
            self.stdout.write(
                self.style.WARNING(
                    f"Preserved operational fields for {preserved_variant_count} business-linked variants. "
                    "Stable SKU IDs and transaction history were not recreated."
                )
            )
        if existing_count and not refresh_demo:
            self.stdout.write(
                "Existing products and their variants/images/specifications were preserved. "
                "Use --refresh-demo only when you intentionally want to refresh demo presentation data."
            )
